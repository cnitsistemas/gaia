...
import {
  mapCreateFrequencyResponse,
  mapFrequencyDataResponse,
  mapMakeFrequencyResponse,
} from "../redux/mappers/frequency";
...
const createFrequencyService = async (data) => {
  try {
    ...
    return {
      success: false,
      message:
        "A rota selecionada não possui alunos cadastrados ou não possui alunos para o horário escolido.",
    };
  }
};

const editFrequencyService = async ({ id, data }) => {
  try {
    const item = await AsyncStorage.getItem("auth");
    const auth = JSON.parse(item);
    const token = auth && auth.accessToken;

    const url = `api/frequencias/${id}`;
    ...
    return {
      success: false,
      message: "Falha ao atualizara frequência!",
    };
  }
};
