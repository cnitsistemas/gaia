...
function PerformFrequency(props) {
  const { frequency, getDataFrequency, makeFrequency, editFrequency } = props;
  ...
  useEffect(() => {
    ...
    if (frequency && frequency.id) {
      getDataFrequency({ id: frequency.id })
        .then((res) => {
  ...
  const handleApllyFrequency = () => {
    editFrequency({
      id: frequency.id,
      data: {
        rota_id: frequency.rota_id,
        turno: frequency.shift,
        data_chamada: frequency.dateFrequency,
        realizada: frequency.accomplished,
        sentido: frequency.sense,
        horario: frequency.hour,
        ...
        <>
          <View style={styles.description}>
            <Text style={styles.textDescription}>
              Ao realizar a frequência você deve marcar os alunos que se
              encontram presentes no veículo de execulção da rota.
            </Text>
          </View>
          <ScrollView>
            ...
            <Box alignItems="center" w="100%">
              <TextArea
                ...
                onChangeText={(text) => setObservationValue(text)}
                maxW="300"
              />
            </Box>
          </ScrollView>
            ...
            <VStack space={3}>
              <HStack alignItems="center" justifyContent="space-between">
                <Text fontWeight="medium">
                  Ao enviar a frequência você confirma que todos os alunos
                  marcados estão presentes. Não é possivel editar após o envio e
                  o registro pode ser consultado no painel adiministrativo.
                </Text>
              </HStack>
            </VStack>
          </>
